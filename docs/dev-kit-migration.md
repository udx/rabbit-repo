...
integration resolution to `.rabbit/repo.yaml` and retire the old file through
...
`rabbit.ci` generates the compact `.rabbit/repo.yaml` resolution. A future
adapter may surface the resolution in generated context, but neither tool
should take over the other's job. `rabbit.ci` does not read, rewrite, or delete
the legacy file.
